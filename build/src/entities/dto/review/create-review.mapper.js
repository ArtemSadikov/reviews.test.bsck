"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.CreateReviewMapper = void 0;
const review_title_entity_1 = require("../../domain/review/review-title/review-title.entity");
const review_entity_1 = require("../../domain/review/review.entity");
const create_review_dto_1 = require("./create-review.dto");
class CreateReviewMapper {
    toDomain(entity) {
        return new review_entity_1.ReviewEntity(entity.id, new review_title_entity_1.ReviewTitleEntity(entity.title));
    }
    toEntity(domain) {
        const result = new create_review_dto_1.CreateReviewDto();
        result.id = domain.id;
        result.title = domain.title.value;
        return result;
    }
}
exports.CreateReviewMapper = CreateReviewMapper;
//# sourceMappingURL=create-review.mapper.js.map